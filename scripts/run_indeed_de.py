#!/usr/bin/env python3
"""Scrape indeed.de, merge with master, regenerate exports."""
...
from src.scraper.indeed_de import DEFAULT_SEARCHES, IndeedDeScraper
...
def append_scrape_issues(
    ...
    *,
    ...
    lines = [
        ...
        f"- **Anti-bot:** {investigation.get('anti_bot_note')}",
...
def main() -> int:
    ...
    logger.info(
        "Within indeed source: %d -> %d (removed %d)",
    ...
    report_path = data_dir / "samples" / "indeed_de_scrape_report.json"
    summary = {
    ...
    if not args.no_merge:
        ...
        logger.info(
            "Merged master: %d existing + %d new indeed -> %d",
            len(existing_all),
    ...
    update_background_status(
        data_dir,
        "COMPLETED",
        f"Scrape indeed.de selesai: {sum(r.scraped for r in reports)} listing, "
        f"{cross_stats.unique_new} unik baru (master total {merged_total}).",
    )

    print("\n=== LAPORAN SCRAPE INDEED.DE ===")
