#!/usr/bin/env python3
"""Scrape suche.ausbildung.nrw, merge with master, regenerate exports."""
...
from src.scraper.ausbildung_nrw import (
    AusbildungNrwScraper,
    DEFAULT_SEARCHES,
)
...
logger = logging.getLogger("run_ausbildung_nrw")
...
def write_progress(
    ...
    *,
    ...
    progress_path = data_dir / "progress_ausbildung_nrw.json"
    ...
    lines = [
        ...
        "",
        "| Category | Apprenticeship ID | Discovered | Scraped | Failed | Cross-dup | Unique new |",
        "|----------|-------------------|------------|---------|--------|-----------|------------|",
    ]
    for report in reports:
        cat_stats = cross_stats.by_category.get(report.category_id, {})
        lines.append(
            f"| {report.category_id} | {report.apprenticeship_id} | "
            f"{report.discovered_urls} | {report.scraped} | {report.failed} | "
    ...
    lines.extend(
        [
            "",
            "Progress JSON: `data/progress_ausbildung_nrw.json`",
            "Exports: `data/exports/ausbildung_nrw_*.json`",
            "",
        ]
    )
    ...
    marker = "## suche.ausbildung.nrw Scraper"
...
def append_scrape_issues(
    ...
    *,
    ...
    lines = [
        f"## suche.ausbildung.nrw — {now}",
        "",
        "### Investigation",
        "",
        f"- **API available:** {investigation.get('api_available')}",
        f"- **Method:** {investigation.get('method')}",
        f"- **Apprenticeship IDs:** AE=322, DPA=430 (user URL `527,322` maps to dual study + AE, not DPA)",
        "",
        "### Results",
        "",
    ...
    for report in reports:
        ...
        lines.append(
            ...
            f"{report.failed} | {cat_stats.get('cross_duplicates', 0)} | "
            f"{cat_stats.get('unique_new', 0)} |"
        )
    lines.extend(
        [
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape suche.ausbildung.nrw listings")
    ...
    update_background_status(
        data_dir,
        "RUNNING",
        "Scrape suche.ausbildung.nrw sedang berjalan. Log: `logs/ausbildung_nrw_scrape.log`",
    )

    scraper = AusbildungNrwScraper(
    ...
    logger.info(
        "Within NRW source: %d -> %d (removed %d)",
    ...
    investigation = {
        ...
        "discovery_api": "https://api.ausbildung.nrw/api/user/companies?ids=",
        "apprenticeship_ids": {"ae": 322, "dpa": 430, "dual_study_user_url_527": 527},
        "branch_it": 6,
        "notes": (
            "No public unauthenticated search API; company batches load during virtual scroll. "
            "Detail pages use JSON-LD Organization schema."
        ),
    }

    report_path = data_dir / "samples" / "ausbildung_nrw_scrape_report.json"
    summary = {
    ...
    if not args.no_merge:
        ...
        logger.info(
            "Merged master: %d existing + %d new NRW -> %d",
            len(existing_all),
    ...
    update_background_status(
        data_dir,
        "COMPLETED",
        f"Scrape suche.ausbildung.nrw selesai: {sum(r.scraped for r in reports)} listing, "
        f"{cross_stats.unique_new} unik baru (master total {merged_total}).",
    )

    print("\n=== LAPORAN SCRAPE SUCHE.AUSBILDUNG.NRW ===")
