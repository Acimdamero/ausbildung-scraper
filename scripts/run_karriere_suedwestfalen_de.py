#!/usr/bin/env python3
"""Scrape karriere-suedwestfalen.de, merge with master, regenerate exports."""
...
from src.scraper.karriere_suedwestfalen_de import (
    DEFAULT_SEARCHES,
    KarriereSuedwestfalenDeScraper,
)
...
logger = logging.getLogger("run_karriere_suedwestfalen_de")
...
def write_progress(
    ...
    *,
    ...
    progress = {
        "last_run_at": now,
        "source": "karriere_suedwestfalen_de",
    ...
    progress_path = data_dir / "progress_karriere_suedwestfalen_de.json"
    ...
    lines = [
        "",
        "## karriere-suedwestfalen.de Scraper",
        "",
        f"**Last run:** {now[:19]}",
    ...
    lines.extend(
        [
            "",
            "Progress JSON: `data/progress_karriere_suedwestfalen_de.json`",
            "Exports: `data/exports/karriere_sw_ae_*.json`",
            "",
        ]
    )
    ...
    marker = "## karriere-suedwestfalen.de Scraper"
...
def append_scrape_issues(
    ...
    *,
    ...
    lines = [
        f"## karriere-suedwestfalen.de — {now}",
        "",
        "### Investigation",
        "",
        ...
        f"- **Detail JSON-LD:** {investigation.get('json_ld')}",
        f"- **Expected listings:** {investigation.get('expected_listings')}",
        "",
        "### Results",
        "",
        "| Category | Discovered | Scraped | Failed | Skipped | Cross-dup | Unique new |",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape karriere-suedwestfalen.de listings")
    ...
    update_background_status(
        data_dir,
        "RUNNING",
        "Scrape karriere-suedwestfalen.de sedang berjalan. "
        "Log: `logs/karriere_suedwestfalen_de_scrape.log`",
    )

    scraper = KarriereSuedwestfalenDeScraper(
    ...
    logger.info(
        "Within karriere-suedwestfalen source: %d -> %d (removed %d)",
    ...
    investigation = {
        ...
        "json_ld": "JobPosting schema on /ausbildung/* detail pages",
        "expected_listings": "~84 unique for fachinformatiker+anwendungsentwicklung",
        "notes": (
            "Regional IHK Südwestfalen portal. referenznummer KSW-{id}; "
            "beruf_typ via specialization.py enrichment."
        ),
    }

    report_path = data_dir / "samples" / "karriere_suedwestfalen_de_scrape_report.json"
    summary = {
    ...
    if not args.no_merge:
        ...
        logger.info(
            "Merged master: %d existing + %d new KSW -> %d",
            len(existing_all),
    ...
    update_background_status(
        data_dir,
        "COMPLETED",
        f"Scrape karriere-suedwestfalen.de selesai: {sum(r.scraped for r in reports)} listing, "
        f"{cross_stats.unique_new} unik baru (master total {merged_total}).",
    )

    print("\n=== LAPORAN SCRAPE KARRIERE-SUEDWESTFALEN.DE ===")
    ...
    print(f"Skip (Beruf salah)  : {sum(r.skipped_wrong_beruf for r in reports)}")
