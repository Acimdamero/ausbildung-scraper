#!/usr/bin/env python3
"""Scrape ausbildungsstellen.de, merge with master, regenerate exports."""
...
from src.scraper.ausbildungsstellen_de import (
    DEFAULT_SEARCHES,
    AusbildungsstellenDeScraper,
)
...
logger = logging.getLogger("run_ausbildungsstellen_de")
...
def write_progress(
    ...
    *,
    ...
    progress_path = data_dir / "progress_ausbildungsstellen_de.json"
    ...
    lines.extend(
        [
            "",
            "Progress JSON: `data/progress_ausbildungsstellen_de.json`",
            "Exports: `data/exports/ausbildungsstellen_*_*.json`",
            "",
        ]
    )
    ...
    marker = "## ausbildungsstellen.de Scraper"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape ausbildungsstellen.de listings")
    ...
    update_background_status(
        data_dir,
        "RUNNING",
        "Scrape ausbildungsstellen.de sedang berjalan. Log: `logs/ausbildungsstellen_de_scrape.log`",
    )

    scraper = AusbildungsstellenDeScraper(
    ...
    logger.info(
        "Within ausbildungsstellen source: %d -> %d (removed %d)",
    ...
    investigation = {
        ...
        "detail_url_pattern": "/ausbildung-{slug}-{id}.html OR /job.php?c={chiffre} (redirect)",
        "notes": (
            "Server-rendered HTML; job.php may 302 to backinjob/ausbildung.de. "
            "Native pages include schema.org JobPosting and span.beginn hints."
        ),
    }

    report_path = data_dir / "samples" / "ausbildungsstellen_de_scrape_report.json"
    summary = {
    ...
    if not args.no_merge:
        ...
        logger.info(
            "Merged master: %d existing + %d new ausbildungsstellen -> %d",
            len(existing_all),
    ...
    update_background_status(
        data_dir,
        "COMPLETED",
        f"Scrape ausbildungsstellen.de selesai: {sum(r.scraped for r in reports)} listing, "
        f"{cross_stats.unique_new} unik baru (master total {merged_total}).",
    )

    print("\n=== LAPORAN SCRAPE AUSBILDUNGSSTELLEN.DE ===")
