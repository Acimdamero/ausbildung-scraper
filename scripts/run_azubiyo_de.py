#!/usr/bin/env python3
"""Scrape azubiyo.de, merge with master, regenerate exports."""
...
from src.scraper.azubiyo_de import AzubiyoDeScraper, DEFAULT_SEARCHES
...
logger = logging.getLogger("run_azubiyo_de")
...
def write_progress(
    ...
    *,
    ...
    progress_path = data_dir / "progress_azubiyo_de.json"
    ...
    lines.extend(
        [
            "",
            "Progress JSON: `data/progress_azubiyo_de.json`",
            "Exports: `data/exports/azubiyo_de_*.json`",
            "",
        ]
    )
...
def append_scrape_issues(
    ...
    *,
    ...
    lines = [
        ...
        f"- **start=20262:** {investigation.get('start_param_note')}",
        "",
        "### Results",
        "",
        "| Category | Discovered | Scraped | Failed | Skipped | Cross-dup | Unique new |",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape azubiyo.de listings")
    ...
    update_background_status(
        data_dir,
        "RUNNING",
        "Scrape azubiyo.de sedang berjalan. Log: `logs/azubiyo_de_scrape.log`",
    )

    scraper = AzubiyoDeScraper(
    ...
    logger.info(
        "Within azubiyo source: %d -> %d (removed %d)",
    ...
    investigation = {
        ...
        "start_param_note": "start=20262 filters Ausbildungsbeginn 2026 (site shows 532 total, ~98 match with subject+radius filters)",
        "detail_url_pattern": "https://www.azubiyo.de/stellenanzeigen/{slug}_{company}_{id}/",
        "notes": (
            "Angular SPA; no listing search API intercepted. "
            "Detail pages expose JobPosting schema.org JSON-LD."
        ),
    }

    report_path = data_dir / "samples" / "azubiyo_de_scrape_report.json"
    summary = {
    ...
    if not args.no_merge:
        ...
        logger.info(
            "Merged master: %d existing + %d new azubiyo -> %d",
            len(existing_all),
    ...
    update_background_status(
        data_dir,
        "COMPLETED",
        f"Scrape azubiyo.de selesai: {sum(r.scraped for r in reports)} listing, "
        f"{cross_stats.unique_new} unik baru (master total {merged_total}).",
    )

    print("\n=== LAPORAN SCRAPE AZUBIYO.DE ===")
