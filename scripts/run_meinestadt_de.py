#!/usr/bin/env python3
"""Scrape meinestadt.de, merge with master, regenerate exports."""
...
from src.scraper.meinestadt_de import (
    DEFAULT_SEARCHES,
    MeinestadtDeScraper,
)
...
logger = logging.getLogger("run_meinestadt_de")
...
def write_progress(
    ...
    *,
    ...
    progress_path = data_dir / "progress_meinestadt_de.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape meinestadt.de listings")
    ...
    update_background_status(
        data_dir,
        "RUNNING",
        "Scrape meinestadt.de sedang berjalan. Log: `logs/meinestadt_de_scrape.log`",
    )

    scraper = MeinestadtDeScraper(
    ...
    logger.info(
        "Within meinestadt source: %d -> %d (removed %d)",
    ...
    investigation = {
        ...
        "radius_note": "Deutschland-wide via /deutschland/lehrstellen/jkl/{path_ids}",
        "detail_url_pattern": "/deutschland/lehrstellen/standard?id={jobId}",
        "path_id_mapping": {
            "97268-16197-16203-13659": "Fachinformatiker Anwendungsentwicklung (AE)",
            "97268-16197-16203-7817": "Fachinformatiker Systemintegration (SI)",
            "97268-16360-17489-18734": "All Fachinformatiker (DPA/DV card filter)",
        },
        "notes": (
            "User URL IDs are filter hierarchy, not one ID per specialization. "
            "DPA/DV lack dedicated jkl paths; scraped from all-FI URL with card hints."
        ),
    }

    report_path = data_dir / "samples" / "meinestadt_de_scrape_report.json"
    summary = {
    ...
    if not args.no_merge:
        ...
        logger.info(
            "Merged master: %d existing + %d new meinestadt -> %d",
            len(existing_all),
    ...
    update_background_status(
        data_dir,
        "COMPLETED",
        f"Scrape meinestadt.de selesai: {sum(r.scraped for r in reports)} listing, "
        f"{cross_stats.unique_new} unik baru (master total {merged_total}).",
    )

    print("\n=== LAPORAN SCRAPE MEINESTADT.DE ===")
