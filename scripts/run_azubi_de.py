#!/usr/bin/env python3
"""Scrape azubi.de, merge with master, regenerate exports."""
...
from src.scraper.azubi_de import AzubiDeScraper, DEFAULT_SEARCHES
...
logger = logging.getLogger("run_azubi_de")
...
def write_progress(
    ...
    *,
    ...
    progress_path = data_dir / "progress_azubi_de.json"
    ...
    lines.extend(
        [
            "",
            "**beruf_typ breakdown (azubi.de scraped):**",
            "",
        ]
    )
    ...
    lines.extend(
        [
            "",
            "Progress JSON: `data/progress_azubi_de.json`",
            "Exports: `data/exports/azubi_de_*.json`",
            "",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape azubi.de listings")
    parser.add_argument("--delay", type=float, default=0.3, help="Delay between detail pages")
    ...
    update_background_status(
        data_dir,
        "RUNNING",
        "Scrape azubi.de sedang berjalan. Log: `logs/azubi_de_scrape.log`",
    )

    scraper = AzubiDeScraper(
    ...
    logger.info(
        "Within azubi.de source: %d -> %d (removed %d)",
    ...
    investigation = {
        ...
        "expected_pages": 34,
        "detail_url_pattern": "https://www.azubi.de/ausbildungsplatz/{id}-p-{slug}",
        "notes": (
            "Server-rendered HTML with schema.org JSON-LD. "
            "Combined Fachinformatiker page covers AE/SI/DPA/DV; beruf_typ via specialization.py."
        ),
    }

    report_path = data_dir / "samples" / "azubi_de_scrape_report.json"
    summary = {
    ...
    if not args.no_merge:
        ...
        logger.info(
            "Merged master: %d existing + %d new azubi.de -> %d",
            master_before,
    ...
    update_background_status(
        data_dir,
        "COMPLETED",
        f"Scrape azubi.de selesai: {sum(r.scraped for r in reports)} listing, "
        f"{cross_stats.unique_new} unik baru (master total {merged_total}).",
    )

    print("\n=== LAPORAN SCRAPE AZUBI.DE ===")
