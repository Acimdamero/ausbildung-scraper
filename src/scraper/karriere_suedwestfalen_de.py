"""Playwright scraper for karriere-suedwestfalen.de jobboerse + detail pages."""
...
from src.parser.karriere_suedwestfalen_de_parser import KarriereSuedwestfalenDeParser

logger = logging.getLogger(__name__)

BASE_URL = "https://www.karriere-suedwestfalen.de"

DEFAULT_SEARCHES: dict[str, dict[str, Any]] = {
    "karriere_sw_ae": {
        "search_url": (
            "https://www.karriere-suedwestfalen.de/jobboerse"
            "?s=fachinformatiker+anwendungsentwicklung"
        ),
    },
}

LISTING_PATH_RE = re.compile(r'href="(/ausbildung/[^"]+-\d+)"')
MAX_PAGES = 60
GOTO_RETRIES = 4
...
class KarriereSuedwestfalenDeScraper:
    """Discover /ausbildung/ links via paginated jobboerse, then scrape detail pages."""
    ...
    def __init__(
        ...
        *,
        ...
        parser: KarriereSuedwestfalenDeParser | None = None,
        progress_path: Path | None = None,
        user_agent: str | None = None,
    ) -> None:
        self.headless = headless
        self.request_delay = max(0.0, request_delay)
        self.parser = parser or KarriereSuedwestfalenDeParser()
    ...
    def scrape_all(
        ...
        with sync_playwright() as playwright:
            ...
            for category_id, config in targets.items():
                logger.info("Scraping karriere-suedwestfalen.de category %s", category_id)
    ...
    def _discover_urls(self, context: BrowserContext, search_url: str) -> list[str]:
        ...
        query = search_url.split("?", 1)[1] if "?" in search_url else ""
        stagnant = 0
        for page_num in range(1, MAX_PAGES + 1):
            if page_num == 1:
                page_url = search_url
            else:
                page_url = f"{BASE_URL}/jobboerse/{page_num}?{query}"
            try:
                ...
                logger.error("Search page %d failed: %s", page_num, exc)
                break

            links = {f"{BASE_URL}{path}" for path in LISTING_PATH_RE.findall(html)}
            if not links:
                logger.info("Pagination stop at page %d", page_num)
                break
            before = len(all_urls)
