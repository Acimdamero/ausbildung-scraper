"""Playwright scraper for ausbildungsmarkt.de search + detail pages."""
...
from src.parser.ausbildungsmarkt_de_parser import AusbildungsmarktDeParser

logger = logging.getLogger(__name__)

BASE_URL = "https://www.ausbildungsmarkt.de"
...
DEFAULT_SEARCHES: dict[str, dict[str, Any]] = {
    "ausbildungsmarkt_fi": {
        "search_url": (
            "https://www.ausbildungsmarkt.de/suche.html"
            "?q=Fachinformatiker%2Fin&l=&r=100"
        ),
    },
}
...
class AusbildungsmarktDeScraper:
    """Discover listings via paginated search (?p=N), then visit each detail page."""
    ...
    def __init__(
        ...
        *,
        ...
        parser: AusbildungsmarktDeParser | None = None,
        progress_path: Path | None = None,
        user_agent: str | None = None,
    ) -> None:
        self.headless = headless
        self.request_delay = max(0.0, request_delay)
        self.parser = parser or AusbildungsmarktDeParser()
    ...
    def scrape_all(
        ...
        with sync_playwright() as playwright:
            ...
            for category_id, config in targets.items():
                logger.info("Scraping ausbildungsmarkt.de category %s", category_id)
    ...
    @classmethod
    def _extract_listing_urls_from_html(cls, html: str) -> set[str]:
        urls: set[str] = set()
        for href in LISTING_HREF_RE.findall(html):
            urls.add(cls._normalize_detail_url(href))
        return urls
    ...
    def _has_next_page_html(html: str) -> bool:
        ...
        # ausbildungsmarkt.de uses » only; ausbildungsstellen.de uses "Weiter »"
        return "Weiter" in block or "&raquo;" in block or "»" in block
