"""Playwright scraper for aubi-plus.de search + detail pages."""
...
from src.parser.aubi_plus_de_parser import AubiPlusDeParser
...
DEFAULT_SEARCHES: dict[str, dict[str, Any]] = {
    "aubi_plus_fi": {
        "search_url": (
            ...
            "?aSuggestLat=&aSuggestLon=&bAZA=&bAZF=&bAZS=&bAZU="
            "&mSuggest=fachinformatiker+&aSuggest=&fGeo=25"
            "&submit=Suchen&fLand%5B%5D=deutschland&s%5B%5D=relevanz"
        ),
    },
}
...
class AubiPlusDeScraper:
    """Discover ausbildung detail URLs via Playwright pagination (seite=N)."""
    ...
    def __init__(
        ...
        *,
        ...
        parser: AubiPlusDeParser | None = None,
        progress_path: Path | None = None,
        user_agent: str | None = None,
    ) -> None:
        self.headless = headless
        self.request_delay = max(0.0, request_delay)
        self.parser = parser or AubiPlusDeParser()
    ...
    def scrape_all(
        ...
        with sync_playwright() as playwright:
            ...
            for category_id, config in targets.items():
                logger.info("Scraping aubi-plus.de category %s", category_id)
    ...
    def _discover_urls(self, context: BrowserContext, search_url: str) -> list[str]:
        ...
        try:
            ...
            pagination_base = self._pagination_base_url(page, search_url)
            stagnant = 0
            for page_num in range(1, MAX_PAGES + 1):
                page_url = self._page_url(pagination_base, page_num)
                if page_num > 1:
                ...
                if not links:
                    logger.info("Pagination stop at seite %d", page_num)
                    break
                before = len(all_urls)
                all_urls.update(links)
                new_count = len(all_urls) - before
                logger.info(
                    "Search seite %d: %d links (%d new, total %d)",
                    page_num,
                    len(links),
                    new_count,
                    len(all_urls),
    ...
    @staticmethod
    def _pagination_base_url(page: Page, fallback: str) -> str:
        href = page.evaluate(
            """() => {
                const link = document.querySelector('.pagination a[href*="seite=2"]');
    ...
    def _extract_listing_urls(page: Page) -> set[str]:
        hrefs = page.locator("a").evaluate_all(
            """els => [...new Set(els.map(e => e.href).filter(h =>
                h.includes('aubi-plus.de/ausbildung/') && /-\\d+\\/?$/.test(h)
            ))]"""
        )
        return set(hrefs)
