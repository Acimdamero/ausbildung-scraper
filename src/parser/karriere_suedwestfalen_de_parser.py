"""Parse karriere-suedwestfalen.de ausbildung detail pages into AusbildungListing."""
...
SECTION_MARKERS = {
    "tasks": ("was dich erwartet", "deine aufgaben", "das erwartet dich", "das lernst du"),
    "requirements": ("was wir uns von dir", "dein profil", "voraussetzungen", "das bringst du mit"),
    "offers": ("wir bieten", "deine vorteile", "darauf kannst du dich freuen"),
    "contact": ("ansprechpartner", "kontakt"),
}


class KarriereSuedwestfalenDeParser:
    ...
    def parse_detail(
        ...
        *,
        ...
        referenznummer = f"KSW-{listing_id}"

        address = self._job_address(job)
        hiring = (job or {}).get("hiringOrganization") or {}
        company_name = clean_text(str(hiring.get("name", "") or ""))
        if not company_name:
            for line in main_text.splitlines():
                line = line.strip()
                if line and any(m in line for m in ("GmbH", " AG", " KG", " SE")):
                    company_name = line
                    break

        city = clean_text(str(address.get("addressLocality", "") or ""))
        ...
        full_description = self._compose_description(
            ...
            fallback=description or main_text[:4000],
        )
    ...
    def _extract_apply_link(apply_links: list[dict[str, str]], page_url: str) -> str:
        for item in apply_links:
            ...
            if not href or href == page_url:
                continue
            if "jetzt bewerben" in text or "bewerben" in text:
                return href
        return ""
