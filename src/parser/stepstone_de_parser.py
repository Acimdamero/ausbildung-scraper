"""Parse stepstone.de job detail pages into AusbildungListing."""
...
JOB_ID_RE = re.compile(r"--(\d+)(?:-inline)?\.html?$", re.IGNORECASE)
...
class StepstoneDeParser:
    """Map stepstone.de JobPosting JSON-LD to AusbildungListing."""
    ...
    def parse_detail(
        ...
        *,
        ...
        referenznummer = f"STEP-{listing_id}"
        ...
        company_desc = clean_text((company or {}).get("description", "") or "")
    ...
    def _listing_id(url: str) -> str:
        match = JOB_ID_RE.search(url.rstrip("/"))
        if match:
        ...
        if isinstance(hiring, dict):
            for key in ("sameAs", "url"):
                ...
                if isinstance(value, str) and value.startswith("http"):
                    if "stepstone.de" not in value:
                        return value.strip()
        if company:
            for key in ("url", "sameAs"):
                value = company.get(key)
                if isinstance(value, str) and value.startswith("http"):
                    if "stepstone.de" not in value and "/cmp/" not in value:
                        return value.strip()
        return ""
    ...
    def _format_salary(
        ...
        if "gehalt" in blob.lower() or "vergütung" in blob.lower():
    ...
    def _extract_email(
        ...
        *text_blobs: str,
        ...
        for href in mailto_links:
            ...
            if addr and "stepstone.de" not in addr.lower() and not addr.startswith("?"):
                return addr
        for blob in text_blobs:
            match = EMAIL_RE.search(blob or "")
            if match and "stepstone.de" not in match.group(0).lower():
